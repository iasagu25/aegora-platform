...
validate_tenant_id() { [[ "$1" =~ ^[a-z][a-z0-9-]{2,30}$ ]] || fail "Tenant inválido: $1"; }
validate_domain() { [[ "$1" =~ ^[A-Za-z0-9.-]+\.[A-Za-z]{2,}$ ]] || fail "Dominio inválido: $1"; }
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --name) [[ $# -ge 2 ]] || fail "Falta valor para --name"; TENANT_NAME="$2"; shift 2 ;;
    --domain) [[ $# -ge 2 ]] || fail "Falta valor para --domain"; BASE_DOMAIN="$2"; shift 2 ;;
    --admin-email) [[ $# -ge 2 ]] || fail "Falta valor para --admin-email"; ADMIN_EMAIL="$2"; shift 2 ;;
    --stage) [[ $# -ge 2 ]] || fail "Falta valor para --stage"; STAGE="$2"; shift 2 ;;
    --s3-credentials-file) [[ $# -ge 2 ]] || fail "Falta valor para --s3-credentials-file"; S3_CREDENTIALS_FILE="$2"; shift 2 ;;
    --allow-shared-s3-credentials) ALLOW_SHARED_S3_CREDENTIALS=true; shift ;;
    --allow-custom-domain) ALLOW_CUSTOM_DOMAIN=true; shift ;;
...
if [[ "$STAGE" == "publish" || "$STAGE" == "all" ]]; then
  tenant_exists || fail "No se puede publicar: el tenant todavía no está provisionado."
  validate_existing_tenant_identity

  publish_args=(/usr/bin/bash "$PUBLISH_SCRIPT" --tenant "$TENANT")
  [[ "$ALLOW_CUSTOM_DOMAIN" != true ]] || publish_args+=(--allow-custom-domain)
  [[ "$APPLY" != true ]] || publish_args+=(--apply)
