...
function hasJsonOperator(value) {
	if (!value || typeof value !== "object") return false;
	return Object.entries(value).some(([key, child]) => key === "_json" || (Array.isArray(child) ? child.some(hasJsonOperator) : hasJsonOperator(child)));
}
function useItems(collection, query) {
	...
	const fetchAggregate = useMemoize(async (url, filter$1, search$1) => {
		...
		const response = await api.get(url, { params: {
			aggregate,
			...filter$1 ? { filter: serializeFilter(filter$1) } : {},
