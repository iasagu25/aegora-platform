...
if [[ "$CONFIG_LAYOUT" == "managed" ]]; then
  require_file "$TENANT_POSTGRES_SECRETS"
fi
...
if [[ "$CONFIG_LAYOUT" == "legacy" ]]; then
  log "Exportando roles y objetos globales de PostgreSQL (layout legacy)."
  ...
  log "Generando definición de roles exclusiva del tenant '${TENANT_ID}'."
  ...
  {
    printf '%s\n' \
      '-- Aegora tenant-specific PostgreSQL roles' \
      "-- Tenant: ${TENANT_ID}" \
      '-- Generated by backup-tenant.sh' \
      ''

    for role_spec in "${tenant_roles[@]}"; do
      role_name="${role_spec%%:*}"
      role_password="${role_spec#*:}"

      [[ "$role_name" =~ ^[A-Za-z0-9_]+$ ]] ||
        fail "Rol PostgreSQL no permitido: ${role_name}"

      [[ "$role_password" =~ ^[A-Fa-f0-9]+$ ]] ||
        fail "Password PostgreSQL con formato no permitido para ${role_name}"
...
[[ -s "$globals_file" ]] ||
  fail "El fichero de objetos globales/roles está vacío."
